...
DictMmap* parse_mdx_file(const char *path) {
    ...
    for (uint64_t i = 0; i < nrb; i++) {
        ...
        free(comp);
        if (data) { fwrite(data, 1, dlen, tmp); doff += dlen; free(data); }
    }
    fclose(f); free(rb_infos);
    int tmp_fd = fileno(tmp);
    struct stat st; fstat(tmp_fd, &st);
    size_t def_data_size = st.st_size;
    void *def_data = malloc(def_data_size);
    if (!def_data) { for(size_t i=0; i<hw_count; i++) free(hw_entries[i].word); free(hw_entries); fclose(tmp); return NULL; }
    lseek(tmp_fd, 0, SEEK_SET);
    read(tmp_fd, def_data, def_data_size);
    rewind(tmp);
    if (ftruncate(tmp_fd, 0) != 0) perror("[MDX] ftruncate");
    ...
    for (size_t i = 0; i < hw_count; i++) {
        ...
        if (d_start < def_data_size && d_end <= def_data_size && d_end > d_start) {
            tree_entries[i].h_off = ftell(tmp);
            tree_entries[i].h_len = strlen(hw_entries[i].word);
            fwrite(hw_entries[i].word, 1, tree_entries[i].h_len, tmp);
            fwrite("\n", 1, 1, tmp);
            tree_entries[i].d_off = ftell(tmp);
            tree_entries[i].d_len = (size_t)(d_end - d_start);
            fwrite((unsigned char *)def_data + d_start, 1, tree_entries[i].d_len, tmp);
            fwrite("\n", 1, 1, tmp);
        }
    }
    free(def_data);
    fflush(tmp); fstat(tmp_fd, &st);
    DictMmap *dict = calloc(1, sizeof(DictMmap));
    dict->fd = tmp_fd; dict->tmp_file = tmp; dict->size = st.st_size;
    dict->data = (const char *)mmap(NULL, dict->size, PROT_READ, MAP_PRIVATE, dict->fd, 0);
    if (dict->data == MAP_FAILED) { free(dict); for(size_t i=0; i<hw_count; i++) free(hw_entries[i].word); free(hw_entries); free(tree_entries); fclose(tmp); return NULL; }
    dict->index = splay_tree_new(dict->data, dict->size);
    insert_balanced(dict->index, tree_entries, 0, (int)hw_count - 1);
    free(tree_entries);
    printf("[MDX] Indexed %zu headwords\n", hw_count);

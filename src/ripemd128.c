/* ripemd128.c — Pure C RIPEMD-128 implementation
 * Based on goldendict-ng/src/dict/utils/ripemd.cc
 * Originally from libavutil by Michael Niedermayer, James Almer, Zhe Wang
 * GPL v3 */
...
#define rol(v, bits) (((v) << (bits)) | ((v) >> (32 - (bits))))
...
static void ripemd128_transform(RIPEMD128_CTX *ctx, const uint8_t buf[64]) {
    ...
    ctx->state[2] = ctx->state[3] + a + f;
    ctx->state[3] = ctx->state[0] + b + g;
    ctx->state[0] = h;
}
...
void ripemd128_digest(RIPEMD128_CTX *ctx, uint8_t digest[16]) {
    ...
    write_le32(finalcount, (uint32_t)(bits & 0xFFFFFFFF));
    write_le32(finalcount + 4, (uint32_t)(bits >> 32));

    ripemd128_update(ctx, (const uint8_t *)"\200", 1);
    while ((ctx->count & 63) != 56)
        ripemd128_update(ctx, (const uint8_t *)"", 1);
    ripemd128_update(ctx, finalcount, 8);

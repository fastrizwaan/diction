...
char* dict_render_shared_styles(int dark_mode, const char *theme_name, const char *font_family, int font_size) {
    ...
    buf_append_str(&b,
        ...
        "  --redlink: "); buf_append_str(&b, dark_mode ? "#ff6c6c" : "#ba0000"); buf_append_str(&b, ";\n"
    ...
    buf_append_str(&b, ";line-height:1.45;color:var(--body-color);background:var(--bg-color);margin:0;padding:8px 20px 8px 8px;}"
        ...
        "a.new, a.new *, a[href*=\"redlink=1\"], a[href*=\"redlink=1\"] *, .dict-link.new, .dict-link[href*=\"redlink=1\"]{color:var(--redlink) !important;}");
    ...
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-def-lvl-3::before { display: inline-block; min-width: 1.2em; counter-increment: xdxf-lvl2; content: counter(xdxf-lvl2, decimal) \") \"; font-weight: bold; color: ");
    buf_append_str(&b, dark_mode ? "#ff6b6b" : "#d32f2f");
    buf_append_str(&b, "; }\n");
    /* Switch to 1. if depth >= 3 */
    buf_append_str(&b, ".xdxf-profile-strict.xdxf-ar:has(.xdxf-def-lvl-4) .xdxf-def-lvl-3::before { content: counter(xdxf-lvl2, decimal) \". \"; }\n");

    /* Level 3 (lvl-4) Numbering */
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-def-lvl-4::before { display: inline-block; min-width: 1.2em; counter-increment: xdxf-lvl3; content: counter(xdxf-lvl3, decimal) \") \"; font-weight: bold; color: ");
    buf_append_str(&b, dark_mode ? "#ff6b6b" : "#d32f2f");
    buf_append_str(&b, "; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-gr { display: inline-block !important; color: ");
    ...
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-co { display: inline; color: inherit; font-style: italic; opacity: 0.8; margin-left: 0; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-co::before, .xdxf-profile-strict .xdxf-co::after { content: \"\" !important; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-dtrn { display: inline; font-weight: bold; color: ");
    buf_append_str(&b, dark_mode ? "#ff9800" : "#e65100");
    buf_append_str(&b, "; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-deftext { display: inline; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-ex { display: block; padding-left: 1em; margin: 0.2em 0; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-ex_orig { display: block; color: ");
    buf_append_str(&b, dark_mode ? "#b39ddb" : "#673ab7");
    buf_append_str(&b, "; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-ex_tran { display: block; color: ");
    buf_append_str(&b, dark_mode ? "#9e9e9e" : "#757575");
    buf_append_str(&b, "; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-iref { color: ");
    buf_append_str(&b, link_color);
    buf_append_str(&b, "; text-decoration: underline; margin-left: 0.4em; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-categ, .xdxf-profile-strict .xdxf-sr { display: block; margin-top: 0.2em; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-rref { display: inline-block; vertical-align: middle; margin-left: 0; margin-right: 0; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-rref.xdxf-snd { text-decoration: none; font-size: 1.2em; cursor: pointer; }\n");
    buf_append_str(&b, ".xdxf-profile-strict .xdxf-etm { display: block; margin-top: 4px; font-style: italic; opacity: 0.8; }\n");
    ...
    if (dark_mode) {
        buf_append_str(&b,
            ...
            "    if (bg && (bg.startsWith('rgb') || bg === 'white' || bg === 'lightgray')) {\n"
            ...
            "          if (fgRgb && fgRgb.length >= 3) {\n"
            "            const fgR = parseInt(fgRgb[0]), fgG = parseInt(fgRgb[1]), fgB = parseInt(fgRgb[2]);\n"
            "            const fgBrightness = (fgR * 299 + fgG * 587 + fgB * 114) / 1000;\n"
            "            if (fgBrightness < 150) {\n"
            "              el.style.setProperty('color', '#ececec', 'important');\n"
            "            }\n"
            "          }\n"
            "        }\n"
            "      }\n"
            ...
            "    if (borderTopColor && borderTopColor.startsWith('rgb')) {\n"
            "      const brgb = borderTopColor.match(/\\d+/g);\n"
            "      if (brgb && brgb.length >= 3) {\n"
            "        const br = parseInt(brgb[0]), bgG = parseInt(brgb[1]), bb = parseInt(brgb[2]);\n"
            "        const bBrightness = (br * 299 + bgG * 587 + bb * 114) / 1000;\n"
            "        if (bBrightness > 150) {\n"
            "          const darkBorderR = Math.round(br * 0.22);\n"
            "          const darkBorderG = Math.round(bgG * 0.22);\n"
            "          const darkBorderB = Math.round(bb * 0.22);\n"
            "          const darkBorder = 'rgb(' + darkBorderR + ', ' + darkBorderG + ', ' + darkBorderB + ')';\n"
            "          el.style.setProperty('border-color', darkBorder, 'important');\n"
            "        }\n"
            "      }\n"
            "    }\n"
            "  }\n"
            ...
            "  document.addEventListener('DOMContentLoaded', makePageDark);\n"
            "  window.addEventListener('load', makePageDark);\n"
            "} else {\n"
            "  makePageDark();\n"
            "}\n"
        );
    ...
    buf_append_str(&b,
        "/* MediaWiki/Wikipedia Layout Styles */\n"
        ".mwiki .floatright, .mwiki table.floatright, .mwiki div.floatright, .mwiki div.tright, .mwiki .infobox, .mwiki .sidebar, .mwiki .vertical-navbox, .mwiki .thumb {\n"
        "  float: right !important;\n"
        "  clear: right !important;\n"
        "  margin: 0.5rem 0 1.25rem 1.4rem !important;\n"
        "}\n"
        ".mwiki .floatleft, .mwiki table.floatleft, .mwiki div.floatleft, .mwiki div.tleft {\n"
        "  float: left !important;\n"
        "  clear: left !important;\n"
        "  margin: 0.5rem 1.4rem 1.25rem 0 !important;\n"
        "}\n"
        ".mwiki div.thumb {\n"
        ...
        "}\n"
        ".mwiki .infobox, .mwiki .sidebar, .mwiki .vertical-navbox {\n"
        "  border: 1px solid var(--table-border) !important;\n"
        "  background-color: var(--code-bg) !important;\n"
        "  font-size: 0.9em !important;\n"
        ...
        "}\n"
        ".mwiki .infobox table, .mwiki .sidebar table {\n"
        "  border-collapse: collapse !important;\n"
        "  width: 100% !important;\n"
        "}\n"
        ".mwiki .infobox td, .mwiki .infobox th, .mwiki .sidebar td, .mwiki .sidebar th {\n"
    ...
    if (dark_mode) {
        buf_append_str(&b,
            ...
            ".infobox, .vertical-navbox, .navbox, .sisterproject, .sister-project, .mw-sidebar, .sidebar, "
            ...
            ".sisterproject td, .sisterproject th, .sidebar td, .sidebar th, "
            "table.infobox, table.navbox, table.wikitable, table.sisterproject, table.sidebar, "
            ".mw-collapsed, .mw-collapsible, .toc, .toccolours, .sidebar-subgroup, "
            ".sistersitebox, .side-box, .side-box-right, .sidebar-list, "
            ".toccolours td, .toccolours th, .year-navigation-box, .year-navigation-box td, .year-navigation-box th, "
            ".year-navigation, .year-navigation td, .year-navigation th,\n"
            "table, tr, td, th, blockquote, .quotebox, .templatequote, .quote, .quote-box, .bq,\n"
            "div[style*=\"background-color\"], table[style*=\"background-color\"], td[style*=\"background-color\"], th[style*=\"background-color\"], blockquote[style*=\"background-color\"] {\n"
            ...
            ".sidebar-title, .sidebar-heading, .sidebar-pretitle, .sidebar-above, .sidebar-below, "
            ".sidebar-list-title, .sidebar-list-title-c, .mwiki th, .vevent th, .plainlinks th {\n"
            "  background-color: #2e2e32 !important;\n"
            "  background: #2e2e32 !important;\n"
            "  color: var(--heading-color) !important;\n"
            "}\n"
            "/* Wikipedia Portal Current Events Layout Alignment */\n"
            ".p-current-events-main {\n"
            "  width: 100% !important;\n"
            "  border: none !important;\n"
            "  background: transparent !important;\n"
            "}\n"
            ".p-current-events-main tr {\n"
            ...
            "  border: none !important;\n"
            "}\n"
            ".infobox a, .navbox a, .wikitable a, .sisterproject a, .sister-project a, .mw-sidebar a, .sidebar a, "
            ".mw-collapsible a, .toc a, .sistersitebox a, .side-box a, .sidebar-list a, "
            ".toccolours a, .year-navigation-box a, .year-navigation a,\n"
            "table a, tr a, td a, th a, blockquote a, .quotebox a, .templatequote a, .quote a, .quote-box a, .bq a {\n"
            "  color: var(--link-color) !important;\n"
            "}\n"
            "/* Reset any inline text colors to match our theme in these blocks */\n"
            ".infobox *, .navbox *, .sisterproject *, .sister-project *, .mw-sidebar *, .sidebar *, .toc *, .thumb *, "
            ".sistersitebox *, .side-box *, .sidebar-list *, .toccolours *, .year-navigation-box *, .year-navigation *,\n"
            "table *, tr *, td *, th *, blockquote *, .quotebox *, .templatequote *, .quote *, .quote-box *, .bq * {\n"
            "  color: inherit;\n"
            "}\n"
            "/* Re-apply links inside them */\n"
            ".infobox a *, .navbox a *, .sisterproject a *, .sister-project a *, .mw-sidebar a *, .sidebar a *, .toc a *, "
            ".sistersitebox a *, .side-box a *, .sidebar-list a *, .toccolours a *, .year-navigation-box a *, .year-navigation a *,\n"
            "table a *, tr a *, td a *, th a *, blockquote a *, .quotebox a *, .templatequote a *, .quote a *, .quote-box a *, .bq a * {\n"
            "  color: var(--link-color) !important;\n"
            "}\n"
            "/* Red links override */\n"
            "a.new, a.new *, a[href*=\"redlink=1\"], a[href*=\"redlink=1\"] *, .dict-link.new, .dict-link[href*=\"redlink=1\"] {\n"
            "  color: var(--redlink) !important;\n"
            "}\n"
        );
    }
